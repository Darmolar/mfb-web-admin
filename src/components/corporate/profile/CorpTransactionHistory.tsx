...
export function CorpTransactionHistory({ entity }: Props) {
  const txs = mockTransactions.filter(t => t.corporateId === entity.id)
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-100">
              {['Reference ID', 'Type', 'Amount', 'Beneficiary', 'Channel', 'Date', 'Status'].map(h => (
